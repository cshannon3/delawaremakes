...
class _MapsPage2State extends State<MapsPage2> {
  ...
  //     //     RequestModelCount r = RequestModelCount(model);
  //     //     r.init();//  if (r.data != {})
  //     //     requests.add(r); //  else print(value);
  //     //   });
  //     //   requests.sort((a, b) => b.remaining().compareTo(a.remaining()));
  //     });
  //   });
  ...
  Widget build(BuildContext context) {
    ...
    List orgsData = dataRepo.getItemsWhere("orgs", fieldFilters: {"isVerified": true});
            ...
            child: Column(
              children: <Widget>[
                ...
                selected == "List"
                    ...
                    : mapWidget(dropOffData, pickUpData)
              ],
            ),
          )
        : Padding(
  ...
  Widget optionsBar(List orgsData, List dropOffData, List pickUpData) {
    List<RequestModelCount> remaining =
        requests.where((requestModel) => requestModel.remaining() > 0 &&requestModel.request.getVal("isVerified", alt: false)).toList();
    remaining
        .sort((a, b) => b.quantityRequested.compareTo(a.quantityRequested));
    List<RequestModelCount> closed =
        requests.where((requestModel) => requestModel.remaining() <= 0 &&requestModel.request.getVal("isVerified", alt: false)).toList();
    ...
    return Expanded(
        child: Padding(
            ...
            child: ListView(children: [
              ...
              // filterBar(),
              ...remaining.map((requestModel) => RequestTile(requestModel:requestModel)
              ...
              ...requests
                  ...
                  .map((requestModel) => RequestTile(requestModel:requestModel,))
            ])));
  }

  Widget mapWidget(List dropOffData, List pickUpData) {
    return Expanded(
      child: Column(
