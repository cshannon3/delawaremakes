...
class _RootPageState extends State<RootPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: // (appState.hideAppBar())? null:
            appbar(context, widget.currentRoute,
                Theme.of(context).textTheme.subtitle1),
        body: (appState.loading || !appState.isReady)
  ...
  Widget bodyPadding(Widget screen, Size s) {
    ...
    return Container(
        child: Center(
      child:
          Container(width: centerWidth, height: double.infinity, child: screen),
    ));
  }
    ...
    return appState.loggedIn
        ? [
            FlatButton(
                ...
                child: Text(
                    appState.currentUser.getVal("name", alt: "").toUpperCase(),
  ...
  menuButtonClicked(BuildContext context, String route) {
    if (route != widget.currentRoute) tappedMenuButton(context, route);
  }

  appbar(BuildContext context, String currentRoute, TextStyle textStyle) =>
      AppBar(
        ...
        title: Row(
          ...
          children: [
            Text("Delaware Makes", style: textStyle),
          ],
        ),
                ...
                children: <Widget>[
                  MenuButton(
                      ...
                      onPressed: () {
                        ...
                        if (appState.isLoginActive)
                          appState.isLoginActive = false;
                        menuButtonClicked(context, "/");
                      }),
                  MenuButton(
                      name: "Open Requests",
                      onPressed: () {
                  ...
                  MenuButton(
                      ...
                      onPressed: () {
                        ...
                        if (appState.isLoginActive)
                          appState.isLoginActive = false;
                        menuButtonClicked(context, "/aboutus");
                      }),
                  MenuButton(
                      name: "Resources",
                      onPressed: () {
                        ...
                        if (appState.isLoginActive)
                          appState.isLoginActive = false;
                        menuButtonClicked(context, "/designs");
                      }),
                  Expanded(
                    child: Container(),
                  ),
